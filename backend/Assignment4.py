...
# def listAllTermsInIndex(indexref):
#   print_debug("List of terms in lexicon")
#   index_artifact = pt.IndexFactory.of(indexref)
#   lex = index_artifact.getLexicon()
#   count = 0;
#   # maxToDisplay = min(30,lex.numberOfEntries())
#   maxToDisplay = lex.numberOfEntries()
#   # let's just display the first 10 terms, so only loop 10 times (or the length of the list if shorter than 10)
#   index_terms = []
#   for i in range(0,maxToDisplay): 
#     count += 1
#     # print_debug(lex[i].getKey())
#     index_terms.append(lex[i].getKey())
#   return index_terms

# eng_data = pd.read_csv('./source_data/englishsource - Sheet1.csv')
# french_data = pd.read_csv('./source_data/frenchsource - Sheet1.csv')
...
# loaded_df_PS = load_dataframe(eng_data, 'Abstract_PS')
# indexref_PS = build_myindex(loaded_df_PS)
# index_artifact_PS = pt.IndexFactory.of(indexref_PS)
# print_debug(index_artifact_PS.getCollectionStatistics().toString())

# loaded_df_FS = load_dataframe(french_data, 'simplified')
# indexref_FS = build_myindex(loaded_df_FS)
# index_artifact_FS = pt.IndexFactory.of(indexref_FS)
# print_debug(index_artifact_FS.getCollectionStatistics().toString())
